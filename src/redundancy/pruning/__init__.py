...
def register_pruning_method(name: str, selector: Callable[..., PruningPlan]) -> None:
    if not name or name in PRUNING_METHODS:
        raise ValueError(f"Pruning method is empty or already registered: {name!r}")
    PRUNING_METHODS[name] = selector


def select_pruning_plan(method: str, **kwargs) -> PruningPlan:
    try:
        selector = PRUNING_METHODS[method]
    except KeyError as error:
        choices = ", ".join(sorted(PRUNING_METHODS))
        raise ValueError(f"Unknown pruning method {method!r}; choose one of: {choices}") from error
